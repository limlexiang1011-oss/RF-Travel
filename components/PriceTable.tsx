...
export const PriceTable: React.FC<{ onBook: (from: string, to: string) => void, lang: Language }> = ({ onBook, lang }) => {
  ...
  const filteredRoutes = PRICING_MATRIX.filter(r => {
    ...
    if (activeTab === 'KL') return r.from === 'Kuala Lumpur' || r.from === 'City Area' || r.from === 'KLIA';
    return false;
  });
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
      <div className="flex border-b">
        {(['SG', 'JB', 'KL'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-4 text-center font-bold text-lg transition-colors ${activeTab === tab ? 'bg-primary-600 text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}>
              ...
              return (
                <tr key={idx} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-1 py-2 md:px-6 md:py-4 text-center align-middle">
                    <button onClick={() => onBook(route.from, route.to)} className="bg-primary-600 text-white p-1.5 md:px-3 md:py-1 rounded-md shadow-sm">
