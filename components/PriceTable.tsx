          ...
          <table className="w-full text-left border-collapse table-fixed md:table-auto">
            <thead className="text-[10px] md:text-xs text-gray-700 uppercase bg-gray-50/50">
              <tr>
                ...
                <th className="w-[13%] md:w-auto px-1 py-3 md:px-6 text-center align-middle font-bold text-gray-600 tracking-wider"></th>
              </tr>
            </thead>
            <tbody className="text-xs md:text-sm divide-y divide-gray-100">
              {filteredRoutes.map((route, idx) => {
                ...
                return (
                  <tr 
                    key={idx} 
                    className="bg-white hover:bg-primary-50/30 transition-colors duration-200"
                  >
                    <td className="px-2 py-3 md:px-6 font-bold text-gray-800 align-middle whitespace-normal break-words leading-tight text-[11px] md:text-base border-l-4 border-transparent hover:border-primary-500 transition-all pl-1.5 md:pl-5 group-hover:text-primary-700">
                      {labelText.split('\n').map((line, i) => (
